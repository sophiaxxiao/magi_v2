...
def f_vec(t, X, thetas):
    ...
    Parameters:
    1. X - array containing (logS, logI, logR) components. Shape (N x 3).
    2. thetas - array containing (beta, gamma, sigma) parameters.

    Returns:
    Derivatives (dlogS/dt, dlogI/dt, dlogR/dt) as an array of shape (N x 3).
    '''
    # Unpack log-scale components
    logS, logI, logR = tf.unstack(X, axis=1)
    beta = thetas[0]
    gamma = thetas[1]
    sigma = thetas[2]

    # Compute exp values for S, I, R
    S = tf.exp(logS)
    I = tf.exp(logI)
    R = tf.exp(logR)

    # Compute E implicitly: E = 1 - S - I - R (ensures population constraint)
    E = 1.0 - S - I - R
    ...
    # Derivatives on the log scale (chain rule)
    dlogSdt = dSdt / S
    dlogIdt = dIdt / I
    dlogRdt = dRdt / R

    # Return the derivatives as a tensor
    return tf.stack([dlogSdt, dlogIdt, dlogRdt], axis=1)
...
X_obs = np.log(obs_data[["S_obs", "I_obs", "R_obs"]].to_numpy().astype(np.float64)) # E is implicit!
